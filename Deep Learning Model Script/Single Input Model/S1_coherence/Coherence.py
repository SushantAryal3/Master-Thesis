...
def _split_one_zarr(zarr_path, block_size=512, val_frac=0.15, seed=42):
    ...
    blocks = np.unique(root["block_idx"][:])
    blocks = rng.permutation(blocks)
    n_val = max(1, int(round(len(blocks) * val_frac)))
    val_blocks = set(blocks[:n_val].tolist())
    all_block_idx = root["block_idx"][:]
    train_idx = np.where(~np.isin(all_block_idx, list(val_blocks)))[0]
    val_idx   = np.where( np.isin(all_block_idx, list(val_blocks)))[0]
    return train_idx.astype(np.int64), val_idx.astype(np.int64)

def make_multi_splits(zarr_paths, val_frac=0.15, seed=42):
    train_pairs, val_pairs = [], []
    for file_idx, zp in enumerate(zarr_paths):
        tr, va = _split_one_zarr(zp, val_frac=val_frac, seed=seed+file_idx)
...
class MultiZarrChipDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        x = torch.from_numpy(x.copy()).permute(1, 0, 2, 3)
        meta = {
            "sample_idx": local_idx,
            "file_idx"  : file_idx,
            "zarr_name" : self.zarr_paths[file_idx].name,
            "block_idx" : int(root["block_idx"][local_idx]),
            "chip_id"   : str(root["chip_id"][local_idx]),
            "year"      : int(root["year"][local_idx]),
        }
        return x, torch.from_numpy(y.copy()), meta
...
def train(args):
    ...
    if reuse_splits and files_exist:
        ...
        train_pairs, val_pairs = make_multi_splits(zarr_paths, val_frac=val_frac, seed=seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split_dir",     type=str,   default="/home/ucl/elia/aryal/Single Input Model/S1_Coherence/split_dir")
    parser.add_argument("--ckpt_dir",      type=str,   default="/home/ucl/elia/aryal/Single Input Model/S1_Coherence")
    parser.add_argument("--zarr_root", type=str, default="/globalsc/ucl/elia/aryal/Coherence_VV_VH")
