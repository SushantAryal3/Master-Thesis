...
class ptavit3d_ca_dn_features(torch.nn.Module):
    def __init__(self,  in_channels_s2, in_channels_s1, spatial_size_init, nfilters_init=96, nfilters_embed=96, nheads_start=96//4, depths=[2,2,5,2], verbose=True, norm_type='GroupNorm', norm_groups=4, correlation_method='linear',   TimeDim=None,  attention_depth=0.0,stem_norm=True):
        ...
        def resize_scales(channel_size, spatial_size, scales_all):
            ## for our case (64, 128, 128) => (0.667, 0.5, 0.5)
        ...
        # dims_pairs = ((64,64),(64,128),(128,256),(256,512))
        
        # Step 1 of Diagram (Hi Res Feature)
        # S1: Output: (64, T, 128,128)
        # S2: Output: (64, T, 128,128)
        ...
        # if spatial_size_init = (128,128) then it becomes (32,32) expected after stage 1
        spatial_size_init = tuple(ts // 4 for ts in spatial_size_init)
        ## store the encoer stage as it goes downward
        self.stages_dn = [] 
        ## Store the fusion blocks used after both modality branches habe been processed
        self.fuse     = [] 
        # store the cross attention modules used between the two branch at each encoder stage
        self.atts_fuse     = []
        ...
        """
        Stage     channels       nheads        spatial          scales
        0        64 -> 64        24            32 x 32          [8,8,8]
        1        64 -> 128       48            16 x 16          [16,4,4]
        2       128 -> 256       96             8 x 8           [32,2,2]
        3       256 -> 512       192            4 x 4           [64,1,1]
        """
        
        for idx, ((layer_dim_in, layer_dim), layer_depth) in enumerate(zip(dim_pairs, depths)):
            # index (1,2,3,4) = value(24,48,96,192)
            # We can think this as 24 expert looking at different patterns, passed to conv3d.
        ...
        """
        Decoder Stage |  real idx |   channels    | nheads | spatial   | scales
        0                   2        512 -> 256        96     8 x 8      [32,2,2]
        1                   1        256 -> 128        48     16 x 16    [16,4,4]
        2                   0        128 -> 64         24     32 x 32    [8,8,8]
        """
        for idx, ((layer_dim_in, layer_dim), layer_depth) in enumerate(zip(dim_pairs, depths)):
            ...
            scales = scales_all[idx]  # ([32,2,2],[16,4,4],[8,8,8])
            if verbose:
    ...
    def forward(self, input_t1, input_t2):
        conv1_t1 = self.conv1(input_t1) # High Resolution S2
        conv1_t2 = self.conv2(input_t2) # High Resolution S1
        fuse1_hi_res = self.fuse_first(conv1_t1,conv1_t2)  # Fusion of two
        conv1 = self.conv_stem (conv1_t1) # Stage 1/2 of S2
        conv2 = self.conv_stem2(conv1_t2) # stage 1/2 of S1
        fusions   = []
        for idx in range(self.depth):
            conv1 = self.stages_dn[idx](conv1) # Encoder of S2
            conv2 = self.stages_dn2[idx](conv2) # Encoder of S1
            conv1 = conv1 + self.atts_fuse[idx](conv2,conv1) # Cross Attention from S1 to S2
            conv2 = conv2 + self.atts_fuse[idx](conv1,conv2) # Cross Attention from S2 to S1
            fusions = fusions + [self.fuse[idx](conv1,conv2)] # Fusion of S1 and S2 for skip connection
        convs_up = fusions[-1] # Take the last fusion fro the decoder block input
...
class ptavit3dca_dn(torch.nn.Module):
    def __init__(self, in_channels_s2, in_channels_s1, NClasses, nfilters_init=96, nfilters_embed=96, nheads_start=96//4, depths=[2,2,5,2], spatial_size_init=(128,128), verbose=True, norm_type='GroupNorm', norm_groups=4,  correlation_method='mean',segm_act='sigmoid',TimeDim=None, nblocks3d=1,attention_depth=10.0):
        ...
        nblocks3d=nblocks3d # How many block of (MBConv + Attention + FFN) are there inside PTAVITSTage
        self.head3D = torch.nn.Sequential(
            PTAViTStage3DTCHW(
                ...
                TimeDim=TimeDim,                
                depth=attention_depth),
            	Lambda(lambda x: x.mean(dim=2)), # collapse time
            	head_cmtsk(nfilters=nfilters_init, nfilters_embed=nfilters_embed,NClasses=NClasses,
                	norm_type=norm_type,norm_groups=norm_groups) # Predict
        )
...
def make_splits_multi(zarr_paths, block_size=512, val_frac=0.15, seed=42):
    ...
    for zarr_idx, zp in enumerate(zarr_paths):
        ...
        chip_ids = root["meta/chip_id"][:]      

        for i, chip_id in enumerate(chip_ids):
            m = re.match(r"r(\d+)_c(\d+)", str(chip_id))
            if m:
                row = int(m.group(1))
                col = int(m.group(2))
                b   = (row // block_size, col // block_size)
                all_blocks.add(b)
...
def compute_percentiles_if_needed(zarr_path: Path, year: int, cache_dir: Path) -> dict:
    ...
    N             = X.shape[0]
    BATCH         = 256
    print(f"\n  [{year}] No cache found — scanning {N:,} samples for S2 percentiles ...")
...
class MultiZarrChipDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        x_s2 = np.array(root["s2"][chip_idx], dtype=np.float32)  
        x_s2 = x_s2.transpose(1, 0, 2, 3)                       
        for b_idx, b_name in enumerate(BAND_NAMES_S2):
            vmin = percs[b_name]["vmin"]
            vmax = percs[b_name]["vmax"]
            x_s2[b_idx] = np.clip(
                (x_s2[b_idx] - vmin) / (vmax - vmin + 1e-6),
                0.0, 1.0
            )

        x_s1 = np.array(root["s1"][chip_idx], dtype=np.float32)  
        x_s1 = x_s1.transpose(1, 0, 2, 3)                      

        y = np.array(root["y"][chip_idx], dtype=np.float32)       

        if self.transform is not None:
            x_s2, x_s1, y = self.transform(x_s2, x_s1, y)

        chip_id = str(root["meta/chip_id"][chip_idx])
        m = re.match(r"r(\d+)_c(\d+)", chip_id)
        row_off = int(m.group(1)) if m else -1
        col_off = int(m.group(2)) if m else -1
        ...
        return (
            torch.from_numpy(x_s2.copy()),
            torch.from_numpy(x_s1.copy()),
...
def monitor_epoch(model, epoch, val_loader, NClasses, criterion, DEBUG=False):
    ...
    tot_dist  = 0.0
    n_batches = 0
    for idx, (x_s2, x_s1, y, _) in enumerate(val_loader):   
        x_s2 = x_s2.cuda(non_blocking=True)                  
        x_s1 = x_s1.cuda(non_blocking=True)                 
        y    = y.cuda(non_blocking=True)
        with torch.inference_mode(), torch.amp.autocast('cuda'):
            preds = model(x_s2, x_s1)                       
...
def train(args):
    ...
    percs = {}
    for i, zp in enumerate(train_zarr_paths):
        root     = zarr.open_group(str(zp), mode="r")
        year     = int(root.attrs["year"])
        percs[i] = compute_percentiles_if_needed(zp, year=year, cache_dir=CACHE_DIR)
    ...
    model     = ptavit3dca_dn(**model_config).cuda() 
    ...
    for epoch in epoch_pbar:
        ...
        for i, (x_s2, x_s1, y, _) in enumerate(train_loader):
            x_s2 = x_s2.cuda(non_blocking=True)
            x_s1 = x_s1.cuda(non_blocking=True)                 
            y    = y.cuda(non_blocking=True)
        ...
        with open(log_path, "a", newline="") as f:
            csv.writer(f).writerow([
                ...
                segm_metrics["acc"],  segm_metrics["mcc"],
                segm_metrics["kappa"], segm_metrics["precision"],
                segm_metrics["recall"], segm_metrics["iou"], segm_metrics["f1"],
                bound_metrics["acc"], bound_metrics["mcc"],
                bound_metrics["kappa"], bound_metrics["precision"],
                bound_metrics["recall"], bound_metrics["iou"], bound_metrics["f1"],
            ])
        ...
        if improved:
            ...
            patience_counter += 1
            print(f"  No improvement in any metric ({patience_counter}/{patience})")
