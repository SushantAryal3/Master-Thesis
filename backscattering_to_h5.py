...
MANIFEST_PATH = Path(
    "/export/students/aryal/metadata_s1/manifest_cohe_wallonia_descending_sorted_month.json"
)
REF_TIF = Path("/export/students/aryal/WALLONIA_2018-07_8_median_trim.tif")
OUT_ROOT = Path("/export/students/aryal/backscattering_descending_h5")
...
class LRUChipCache:
    ...
    def _open_or_create(self, r0: int, c0: int) -> _ChipHandle:
        ...
        return _ChipHandle(h5f, path, dates, satellites, orbits, angle_paths)
...
def extract_month(
    ...
    buf_bs    = np.empty((chip, chip), dtype=np.float32)  # backscatter band
    buf_angle = np.empty((chip, chip), dtype=np.float32)  # angle band
                ...
                with rasterio.open(angle_path) as src:
                    with WarpedVRT(
                        ...
                        for ci in file_to_chips[fi]:
                            ...
                            # Invalid pixel = backscatter is 0 AND angle is 0
                            # Valid pixel   = backscatter is 0 BUT angle > 0
...
def merge_temp_into_main(
    ...
    for temp_path in tqdm(
        ...
        with h5py.File(main_path, "a") as main_h5, \
            ...
            # ---- CHANGED: merge angle_paths instead of cohe_paths ----
            main_h5.attrs["dates"] = (
            ...
            main_h5.attrs["angle_paths"] = (
                list(main_h5.attrs["angle_paths"]) +
                list(temp_h5.attrs["angle_paths"])
            )
...
def run_pipeline(
    ...
    print("=" * 60)
    print("HDF5 Backscatter Time-Series Pipeline — Month-by-Month")
    print(f"  Years      : {year_start} – {year_end}")
    print(f"  Output     : {out_root}")
    print(f"  Band       : {BACKSCATTER_BAND} (backscatter / sigma0)")
